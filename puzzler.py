...
def generate_puzzles(instream, outstream, engine, variant, req_types, multipv, depth,
                     min_difficulty, max_difficulty, min_quality, min_content, win_threshold, unclear_threshold):
    ...
    for epd in tqdm(instream, total=total):
        ...
        if len(pv) > stm_index:
            ...
            if ((not req_types or types[0] in req_types)
                    and (max_difficulty >= difficulty >= min_difficulty)
                    and (content >= min_content)
                    and (total_quality >= min_quality)):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--min-difficulty', type=float, default=0, help='minimum difficulty')
    parser.add_argument('-x', '--max-difficulty', type=float, default=10, help='maximum difficulty')
    parser.add_argument('-q', '--min-quality', type=float, default=0, help='minimum puzzle quality [0,1]')
    parser.add_argument('-c', '--min-content', type=float, default=-10, help='minimum puzzle content')
    ...
    with fileinput.input(args.epd_files) as instream:
        generate_puzzles(instream, sys.stdout, engine, args.variant, args.types, args.multipv, args.depth,
                         args.min_difficulty, args.max_difficulty, args.min_quality, args.min_content,
